 ...
 *   the utmp file path to be autosearch by locate  -2014/11/13 by kernux
 *
 */
...
/*get the info of ps output use popen function, return by psl_p*/
...
/*get the info of utmp output use utmp file, return by utl_p*/
int fetchutmp(struct utmp_line *utl_p) {
    ...
    if ((f = open(UTMP, O_RDONLY)) > 0) {
        ...
        while (read(f, &ut, sz_ut) > 0 && curp <= endp) {
            if (ut.ut_time == 0) del_cnt++;  /* ut_time shouldn't be zero */
        ...
        close(f);
        if (del_cnt > 0) printf("=> possibly %d deletion(s) detected in %s !\n",
                                del_cnt, UTMP);
    } else {
        fprintf(stderr, "\nfailed opening utmp !\n");
        exit(EXIT_FAILURE);
...
int get_utmp_path()
{
    ...
    int i = 0;
    FILE *fp = popen(cmd, "r");
    while(fgets(lines, 1023, fp))
        i++;
    if (i > 1) {
        fprintf(stderr, "found %d utmp file, and we will use the default path %s\n", i, UTMP);
        fprintf(stderr, "if the default path is not right, use -f option to locate it\n");
        return 0;
    }
    if (1 == i) {
        memset(UTMP, 0, sizeof(UTMP));
        memcpy(UTMP, lines, strlen(lines) - 1);
        return 0;
    }
    fprintf(stderr, "utmp file not found, use default path!\n");
    return 0;
}
