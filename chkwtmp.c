...
int main(int argc, char*argv[]) {
	...
	while (read (filehandle, (char *) &utmp_ent, sizeof (struct utmp)) > 0) {
		if (utmp_ent.ut_tv.tv_sec == 0)
			del_counter++;
		else {
			if (del_counter) {
				printit(del_counter, start_time, utmp_ent.ut_tv.tv_sec);
				t_del++;
				del_counter=0;
			}
			start_time=utmp_ent.ut_tv.tv_sec;
		}
	}
	close(filehandle);
	if (del_counter)
	   printit(del_counter, start_time, act_time);
    return((int) t_del+del_counter);
}
